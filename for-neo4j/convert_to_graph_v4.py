...
def write_csv(data, filename):
    ...
    # 清洗列名：去掉冒号后缀 (id:ID -> id) 方便 Cypher 读取
    clean_cols = {c: c.split(':')[0] for c in df.columns}
...
// 2. Nodes
:auto LOAD CSV WITH HEADERS FROM '{base_url}/nodes_site.csv' AS row MERGE (n:E27_Site {{id: row.id}}) SET n.name=row.name, n.location=row.location;
:auto LOAD CSV WITH HEADERS FROM '{base_url}/nodes_place.csv' AS row MERGE (n:E53_Place {{id: row.id}}) SET n.name=row.name, n.type=row.type;
:auto LOAD CSV WITH HEADERS FROM '{base_url}/nodes_feature.csv' AS row MERGE (n:E25_Man_Made_Feature {{id: row.id}}) SET n.name=row.name, n.code=row.code, n.type=row.type;
:auto LOAD CSV WITH HEADERS FROM '{base_url}/nodes_period.csv' AS row MERGE (n:E4_Period {{id: row.id}}) SET n.name=row.name, n.start_date=row.start_date;
:auto LOAD CSV WITH HEADERS FROM '{base_url}/nodes_artifact.csv' AS row MERGE (n:E22_Man_Made_Object {{id: row.id}}) SET n.name=row.name, n.category=row.category, n.height=toFloat(row.height);
:auto LOAD CSV WITH HEADERS FROM '{base_url}/nodes_production.csv' AS row MERGE (n:E12_Production {{id: row.id}}) SET n.note=row.note;
:auto LOAD CSV WITH HEADERS FROM '{base_url}/nodes_concept.csv' AS row CALL apoc.create.node([row.LABEL], {{id: row.id, name: row.name}}) YIELD node RETURN count(node);

// 3. Edges
:auto LOAD CSV WITH HEADERS FROM '{base_url}/edges_spatial.csv' AS row MATCH (s {{id: row.START_ID}}) MATCH (e {{id: row.END_ID}}) CALL apoc.create.relationship(s, row.TYPE, {{}}, e) YIELD rel RETURN count(rel);
:auto LOAD CSV WITH HEADERS FROM '{base_url}/edges_period.csv' AS row MATCH (s {{id: row.START_ID}}) MATCH (e {{id: row.END_ID}}) MERGE (s)-[:P7_took_place_at]->(e);
:auto LOAD CSV WITH HEADERS FROM '{base_url}/edges_obj_loc.csv' AS row MATCH (s {{id: row.START_ID}}) MATCH (e {{id: row.END_ID}}) MERGE (s)-[:P53_has_former_or_current_location]->(e);
:auto LOAD CSV WITH HEADERS FROM '{base_url}/edges_obj_attr.csv' AS row MATCH (s {{id: row.START_ID}}) MATCH (e {{id: row.END_ID}}) CALL apoc.create.relationship(s, row.TYPE, {{}}, e) YIELD rel RETURN count(rel);
:auto LOAD CSV WITH HEADERS FROM '{base_url}/edges_prod_link.csv' AS row MATCH (s {{id: row.START_ID}}) MATCH (e {{id: row.END_ID}}) MERGE (s)-[:P108i_was_produced_by]->(e);
:auto LOAD CSV WITH HEADERS FROM '{base_url}/edges_prod_attr.csv' AS row MATCH (s {{id: row.START_ID}}) MATCH (e {{id: row.END_ID}}) CALL apoc.create.relationship(s, row.TYPE, {{}}, e) YIELD rel RETURN count(rel);
